...
router.post("/upload/proof/:add_uuid",Upload.upload.single('ss'),(req,res)=>{
    if(req.file_error){
        res.redirect("/p/add/view/"+req.params.add_uuid)
    }else{
        var data = {
            add_uuid:req.params.add_uuid,
            page_uuid:req.user.uuid,
            file:req.file
        }
        Brand.acceptOffer(data,(err,result)=>{
            if(err){
                console.log(err);
            }
            res.redirect("/p/add/view/"+req.params.add_uuid)
