...
router.get('/login',(req,res)=>{
    res.render('auth_login',{msg:null})
})

router.get('/login/err',(req,res)=>{
    res.render('auth_login',{msg:"Incorrect Credentials"})
})

router.get('/signup',(req,res)=>{
    res.render('auth_signup',{msg:null})
})
...
router.get('/signup/err',(req,res)=>{
    ...
    res.render('auth_signup',{msg:"Username Already Exists",redirect:redirect})
})
    ...
    auth.Signup(data,(err,user)=>{
        if(err){
            ...
            passport.authenticate('local',{failureRedirect:"/login"})(req,res,()=>{
                Page.createPage({uuid:user.uuid},(err,result)=>{
                    if(err){
                        console.log(err);
                        res.redirect("/user/home");
                    }else{
                        Brand.createBrand({uuid:user.uuid},(err,result1)=>{
                            if(err){
                                console.log(err);
                                res.redirect("/user/home");
                            }else{
...
router.get('/verifyEmail/ve',(req,res)=>{
    if(req.user){
        res.render('auth_otp',{msg:"Invalid Code"});
    }else{
        res.redirect('/login')
    }
})
...
router.get('/verifyEmail/ss',(req,res)=>{
    if(req.user){
        res.render('auth_otp',{msg:"Verification Code Sent"});
    }else{
        res.redirect('/login')
    }
})

router.get('/verifyEmail/se',(req,res)=>{
    if(req.user){
        res.render('auth_otp',{msg:"Could not send Verification code"});
    }else{
        res.redirect('/login')
    }
})
...
router.get('/forgotPassword',(req,res)=>{
    res.render('auth_forgotPassword',{msg:null,email:null});
})

router.get('/forgotPassword/err',(req,res)=>{
    res.render('auth_forgotPassword',{msg:"No such User",email:null});
})
...
router.get('/confirmPassword/:uuid',(req,res)=>{
    ...
    User.getUser({userid:uuid},(err,user)=>{
        if(user){
            res.render('auth_verifyPassword',{msg:null,user:user});
        }else{
            res.redirect('/login')
        }
    })
...
router.get('/confirmPassword/:uuid/err',(req,res)=>{
    ...
    User.getUser({userid:uuid},(err,user)=>{
        if(user){
            res.render('auth_verifyPassword',{msg:"Incorrect Code",user:user});
        }else{
            res.redirect('/login')
        }
    })
...
router.get('/confirmPassword/:uuid/sc',(req,res)=>{
    ...
    User.getUser({userid:uuid},(err,user)=>{
        if(user){
            res.render('auth_verifyPassword',{msg:"Password Changed Successfully",user:user});
        }else{
            res.redirect('/login')
        }
    })
